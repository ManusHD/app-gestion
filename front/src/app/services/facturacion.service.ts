...
export class FacturacionService {
   ...
   */
  calcularFacturacion(fechaInicio: string, fechaFin: string): Observable<FacturacionCalculada> {
    ...
    console.log("Fecha inicio mes: " + fechaInicioAjustada)
    console.log("Fecha fin mes: " + fechaFinAjustada)
    ...
    // También necesitamos obtener las entradas de ANTES del mes para saber qué había ya almacenado
    const entradasAnteriores$ = this.http.get<any[]>(`${this.apiUrlEntradas}/filtrar`, {
      params: {
        fechaInicio: '2020-01-01', // Fecha muy anterior
        fechaFin: this.restarUnDia(fechaInicioAjustada),
        tipoBusqueda: 'fecha'
      }
    });

    const salidasAnteriores$ = this.http.get<any[]>(`${this.apiUrlSalidas}/filtrar`, {
        params: {
    ...
    return forkJoin([entradas$, salidas$, entradasAnteriores$, salidasAnteriores$]).pipe(
        map(([entradas, salidas, entradasAnteriores, salidasAnteriores]) => {
          ...
          const totalMovimientos = detallesMovimientos.reduce((sum, d) => sum + d.costoTotal, 0);
          const totalAlmacenaje = detallesAlmacenaje.reduce((sum, d) => sum + d.costoTotal, 0);
  ...
  private calcularAlmacenajeCompleto(
    ...
    const inventarioPalets = new Map<string, {
      palets: number;
      fechaUltimaEntrada: Date;
      referencia: string;
      descripcion: string;
      movimientos: Array<{fecha: Date, cantidad: number, tipo: 'entrada' | 'salida'}>;
    }>();
        ...
        entrada.productos.forEach((producto: any) => {
          if (producto.palets && producto.palets > 0) {
            ...
            if (inventarioPalets.has(key)) {
              ...
              item.fechaUltimaEntrada = fechaEntrada > item.fechaUltimaEntrada ? fechaEntrada : item.fechaUltimaEntrada;
              item.movimientos.push({fecha: fechaEntrada, cantidad: producto.palets, tipo: 'entrada'});
            } else {
              inventarioPalets.set(key, {
                ...
                movimientos: [{fecha: fechaEntrada, cantidad: producto.palets, tipo: 'entrada'}]
              });
            }
          }
        });
          ...
          salida.productos.forEach((producto: any) => {
            if (producto.palets && producto.palets > 0) {
              ...
              if (inventarioPalets.has(key)) {
                ...
                item.movimientos.push({fecha: fechaSalida, cantidad: -producto.palets, tipo: 'salida'});
              } else {
                ...
                inventarioPalets.set(key, {
                  ...
                  referencia: producto.ref || '',
                  descripcion: producto.description || '',
                  movimientos: [{fecha: fechaSalida, cantidad: -producto.palets, tipo: 'salida'}]
                });
              }
            }
          });
    ...
    inventarioPalets.forEach((item, key) => {
      ...
      const movimientosDelMes = item.movimientos.filter(mov => 
        mov.fecha >= fechaInicioDate && mov.fecha <= fechaFinDate
      ).sort((a, b) => a.fecha.getTime() - b.fecha.getTime());

      movimientosDelMes.forEach(mov => {
        if (mov.tipo === 'entrada') {
          // Palets que entran: se facturan proporcionalmente desde su entrada
          const diasDesdeEntrada = this.calcularDiasDesdeEntrada(mov.fecha, fechaFinDate);
          const costoEntrada = (mov.cantidad * this.TARIFA_PALET_MES * diasDesdeEntrada) / diasEnElMes;
          ...
          const diasHastaSalida = this.calcularDiasHastaSalida(fechaInicioDate, mov.fecha);
          const diasNoFacturados = diasEnElMes - diasHastaSalida;
          const descuento = (Math.abs(mov.cantidad) * this.TARIFA_PALET_MES * diasNoFacturados) / diasEnElMes;
   ...
   */
  private calcularDiasDesdeEntrada(fechaEntrada: Date, fechaFinMes: Date): number {
   ...
   */
  private calcularDiasHastaSalida(fechaInicioMes: Date, fechaSalida: Date): number {
  ...
  private calcularMovimientos(entradas: any[]): DetalleMovimiento[] {
    ...
    entradas.forEach(entrada => {
      if (entrada.estado && entrada.productos) {
        entrada.productos.forEach((producto: any) => {
          const detalle = this.calcularMovimientoProducto(producto, entrada.fechaRecepcion);
          if (detalle) {
   ...
   */
  private calcularMovimientoProducto(producto: any, fechaMovimiento: string): DetalleMovimiento | null {
   ...
   */
  private calcularAlmacenajeLegacy(entradas: any[], salidas: any[], fechaInicio: string, fechaFin: string): DetalleAlmacenaje[] {
    const detalles: DetalleAlmacenaje[] = [];
    const paletsAlmacenaje = new Map<string, {
      palets: number;
      fechaEntrada: Date;
      fechaSalida?: Date;
      referencia: string;
    ...
    entradas.forEach(entrada => {
      if (entrada.estado && entrada.productos && entrada.fechaRecepcion) {
        ...
        entrada.productos.forEach((producto: any) => {
          if (producto.palets && producto.palets > 0) {
            const key = `${producto.ref}-${producto.description}-${fechaEntrada.getTime()}`;
        ...
        salida.productos.forEach((producto: any) => {
          if (producto.palets && producto.palets > 0) {
            ...
            paletsAlmacenaje.forEach((value, key) => {
              if (value.referencia === producto.ref && 
    ...
    paletsAlmacenaje.forEach(palet => {
      ...
      if (diasAlmacenaje > 0) {
        const diasEnElMes = this.getDiasEnMes(fechaInicioDate);
        const costoProporcional = (this.TARIFA_PALET_MES * diasAlmacenaje / diasEnElMes) * palet.palets;
  ...
  private calcularDiasAlmacenaje(
    ...
    const inicioAlmacenaje = fechaEntrada > fechaInicioMes ? fechaEntrada : fechaInicioMes;
    
    // El fin del almacenaje es la fecha más temprana entre la salida y el fin del mes
    const finAlmacenaje = fechaSalida && fechaSalida < fechaFinMes ? fechaSalida : fechaFinMes;
    ...
    const diferenciaTiempo = finAlmacenaje.getTime() - inicioAlmacenaje.getTime();
  ...
  exportarCSV(facturacion: FacturacionCalculada, periodo: string): void {
    ...
    // Agregar totales
    rows.push(['', '', '', '', '', 'TOTAL MOVIMIENTOS:', facturacion.totalMovimientos.toFixed(2), '', '']);
    rows.push(['', '', '', '', '', 'TOTAL ALMACENAJE:', facturacion.totalAlmacenaje.toFixed(2), '', '']);
    rows.push(['', '', '', '', '', 'TOTAL GENERAL:', facturacion.totalGeneral.toFixed(2), '', '']);
  ...
  exportarPDF(facturacion: FacturacionCalculada, periodo: string): void {
    ...
    doc.text('FACTURACIÓN MENSUAL', pageWidth / 2, yPosition, { align: 'center' });
    
    yPosition += 15;
    doc.setFontSize(14);
    doc.setFont('helvetica', 'normal');
    doc.text(`Período: ${this.formatearPeriodo(periodo)}`, pageWidth / 2, yPosition, { align: 'center' });
    ...
    if (facturacion.detallesMovimientos.length > 0) {
      ...
      autoTable(doc, {
        startY: yPosition,
        head: [['Referencia', 'Descripción', 'Tipo', 'Cantidad', 'P. Unit.', 'Total']],
    ...
    const tarifasData = [
      ...
      ['Movimiento por unidad', `${this.TARIFA_MOVIMIENTO_UNIDAD.toFixed(2)} €`]
    ];
    ...
    doc.text(`Documento generado el ${new Date().toLocaleDateString('es-ES')}`, 20, finalY);
  ...
  exportarExcel(facturacion: FacturacionCalculada, periodo: string): void {
    ...
    if (facturacion.detallesAlmacenaje.length > 0) {
      const almacenajeHeaders = ['Referencia', 'Descripción', 'Palets', 'Días Almacenaje', 'Costo Total', 'Fecha Entrada', 'Fecha Salida'];
      const almacenajeData = facturacion.detallesAlmacenaje.map(detalle => [
        detalle.referencia,
        detalle.descripcion,
        detalle.palets,
        ...
        detalle.fechaSalida ? detalle.fechaSalida.toLocaleDateString('es-ES') : ''
      ]);

      const almacenajeWS = XLSX.utils.aoa_to_sheet([almacenajeHeaders, ...almacenajeData]);
      almacenajeWS['!cols'] = [
    ...
    if (facturacion.detallesMovimientos.length > 0) {
      const movimientosHeaders = ['Referencia', 'Descripción', 'Tipo', 'Cantidad', 'Precio Unitario', 'Costo Total', 'Fecha Movimiento'];
      const movimientosData = facturacion.detallesMovimientos.map(detalle => [
        detalle.referencia,
        detalle.descripcion,
        detalle.tipo,
      ...
      const movimientosWS = XLSX.utils.aoa_to_sheet([movimientosHeaders, ...movimientosData]);
      movimientosWS['!cols'] = [
        { width: 15 }, { width: 30 }, { width: 12 }, { width: 10 }, 
