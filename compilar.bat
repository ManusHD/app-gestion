...
start cmd /k "cd %MICROSERVICIO_PRODUCTOS% && mvn clean package -DskipTests"

:: Ejecutar entradas en una nueva ventana   PORT = 8092
start cmd /k "cd %MICROSERVICIO_ENTRADAS% && mvn clean package -DskipTests"

:: Ejecutar salidas en una nueva ventana   PORT = 8093
start cmd /k "cd %MICROSERVICIO_SALIDAS% && mvn clean package -DskipTests"

:: Ejecutar ubicaciones en una nueva ventana   PORT = 8095
start cmd /k "cd %MICROSERVICIO_UBICACIONES% && mvn clean package -DskipTests"

:: Ejecutar agencias de envío en una nueva ventana   PORT = 8096
start cmd /k "cd %MICROSERVICIO_AGENCIASENVIO% && mvn clean package -DskipTests"

:: Ejecutar agencias de envío en una nueva ventana   PORT = 8097
start cmd /k "cd %MICROSERVICIO_DIRECCIONES% && mvn clean package -DskipTests"

:: Ejecutar dcs en una nueva ventana   PORT = 8098
start cmd /k "cd %MICROSERVICIO_AUTH% && mvn clean package -DskipTests" 
